...
FAST_PATH_EVIDENCE = ROOT / "evaluations/20260923-fast-path"
...
class EvaluationEvidence(unittest.TestCase):
    ...
    def test_fast_path_matrix_and_acceptance_match(self):
        cases = json.loads((EVIDENCE / "cases.json").read_text())["cases"]
        rows = json.loads((FAST_PATH_EVIDENCE / "results.json").read_text())["rows"]
        summary = json.loads((FAST_PATH_EVIDENCE / "summary.json").read_text())

        self.assertEqual(len(rows), 120)
        self.assertEqual(
            len({(row["case"], row["repetition"], row["arm"]) for row in rows}),
            120,
        )
        self.assertEqual({row["case"] for row in rows}, {case["id"] for case in cases})
        self.assertTrue(summary["planned_attempts_complete"])
        self.assertTrue(summary["candidate_accepted"])
        ...
        self.assertEqual(candidate["artifact_audit_passed"], candidate["valid"])
        self.assertEqual(candidate["sources_preserved"], candidate["attempts"])
        self.assertGreater(summary["improvement"]["median_seconds_percent"], 30)
        self.assertGreater(summary["improvement"]["mean_total_tokens_percent"], 30)
        self.assertGreater(summary["improvement"]["mean_model_calls_percent"], 30)

    def test_fast_path_results_are_sanitized(self):
        text = (FAST_PATH_EVIDENCE / "results.json").read_text()
        github_prefix = r"github" + r"_pat_"
        self.assertNotRegex(text, re.compile(github_prefix + r"|gh[pousr]_[A-Za-z0-9_]{20,}"))
        self.assertNotRegex(text, re.compile(r"sk-[A-Za-z0-9_.-]{20,}"))
        self.assertNotIn('"access_token"', text)
